...
def calc_penalty_stddev(stddev_KD, variance_penalty_mode, variance_penalty_spread):
    ...
    coef_A = 1.0/(s * m * m)
    coef_B = 1.0/(s * m)
    return ag_np.sum(coef_A * ag_np.log(stddev_KD) + 0.5 * coef_B / ag_np.square(stddev_KD))
...
if __name__ == '__main__':
    ...
    G = 1001
    stddev_G = ag_np.logspace(-2, 2, G) # from 0.01 to 100

    for mm, mode in enumerate(mode_list):
        for spread in spread_list:
            ax_grid[mm].plot(
                ag_np.square(stddev_G),
        ...
        if mm == 0:
            ax_grid[mm].set_ylabel("penalty($\sigma^2$)")
